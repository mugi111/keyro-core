...
assert.deepEqual(
  manifest.files.filter((path) => path.includes("v0.1.0")),
  [],
  "initial package contents must ship only the current protocol version"
);

const currentTypes = manifest.exports["./core-studio/v0.2.0"]?.types;
assert.equal(currentTypes, "./generated/typescript/v0.2.0/core-studio.ts");
const currentRuntime = manifest.exports["./core-studio/v0.2.0"]?.import;
assert.equal(currentRuntime, "./generated/javascript/v0.2.0/core-studio.js");

const currentTypeSource = readFileSync(join(packageRoot, currentTypes), "utf8");
assert.match(currentTypeSource, /KEYRO_PROTOCOL_VERSION = "0\.2\.0"/);
...
assert.match(currentRuntimeSource, /KEYRO_PROTOCOL_VERSION = "0\.2\.0"/);
...
const schema = JSON.parse(readFileSync(join(packageRoot, "schemas/v0.2.0/core-studio.schema.json"), "utf8"));
const snapshotVector = JSON.parse(readFileSync(join(packageRoot, "test-vectors/v0.2.0/snapshot-response.json"), "utf8"));
